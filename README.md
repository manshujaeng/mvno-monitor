...
# 스케줄 동작하지만 보통 저녁 6~9시 사이로 차이가 크다  
2일 지나니 스케줄이 돌기 시작함, 오전10시 스케줄이지만 저녁늦게나 실행됨... 하루에 한번만 돌면 되긴 하니 그냥 사용  

# 알뜰폰허브 오류가 생각보다 자주 발생
구글 스크립트로 run workflow 호출하면 알뜰폰허브 접속이 잘 되는데...  
깃허브 스케줄로 실행하면 반이상이 접속실패 오류 발생    
수동실행과 스케줄 실행 시 서버 영향을 타는듯하다.  
--> 좀 더 운영 후 구글스크립터로 실행할지 고민
